...
class NotesService {
  ...
  addNote({ title, body, tags }) {
    ...
    const isSuccess = this._notes.filter((note) => note.id === id).length === 1;
  ...
  updateNoteById(id, { title, body, tags }) {
  ...
  deleteNoteById(id) {
    ...
    if (noteIndex === -1) {
      throw new Error('Gagal menghapus catatan. Catatan tidak ditemukan');
    }
